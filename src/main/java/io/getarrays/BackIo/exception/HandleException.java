package io.getarrays.BackIo.exception;
...
public class HandleException extends ResponseEntityExceptionHandler implements ErrorController {
    ...
    @Override
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException exception, HttpHeaders httpHeaders, HttpStatusCode statusCode, WebRequest request) {
        log.error(exception.getMessage());
    ...
    public ResponseEntity<HttpResponse> sQLIntegrityConstraintViolationException(SQLIntegrityConstraintViolationException exception) {
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .reason(exception.getMessage().contains("Dublicate entry") ? "Information already exist" : exception.getMessage())
                        .developerMessage(exception.getMessage())
                        .status(BAD_REQUEST)
                ...
                HttpResponse.builder()
                        ...
                        .reason(exception.getMessage() != null ?
                                exception.getMessage().contains("expected 1, actual 0") ? "Record not found"
                                        : exception.getMessage() : "Some error occured")
                        .developerMessage(exception.getMessage())
                        .status(FORBIDDEN)
                        .statusCode(FORBIDDEN.value())
                        .build(), FORBIDDEN
    ...
    public ResponseEntity<HttpResponse> lockedException(LockedException exception) {
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .developerMessage(exception.getMessage())
                        .reason("Your accound is locked, please contact with support team.")
                        .status(FORBIDDEN)
                        .statusCode(FORBIDDEN.value())
                        .build(), FORBIDDEN
        );
